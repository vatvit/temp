...
def save_combined_results(all_results):
    ...
    descriptions = {
        'Summary': [
            ...
            "═══ КЛЮЧЕВЫЕ КОЛОНКИ ═══",
            "",
            "• gap - РАСХОЖДЕНИЕ между ожидаемой и вычисленной разницей",
            "  Формула: gap = expected_diff - computed_diff",
            "  Если gap = 0 → алгоритм объяснил все расхождения",
            "  Если gap ≠ 0 → есть необъяснённые расхождения",
            "",
            "• check_items - СУММА позиций, которые могут объяснять gap",
            "  Если check_items ≈ gap → эти позиции вероятно причина расхождения",
            "  Приоритет: 1) WRONG_DATE+DATE_ANOMALY+БАЗА_NO_DATE",
            "             2) TOLERANCE (если нет проблем с датами)",
            "             3) NOT_FOUND или БАЗА_EXTRA (что больше)",
            "",
            "═══ КАК ВЫЧИСЛЯЕТСЯ ═══",
            "",
            "• sold_tovar - сумма продаж в 'tovar' (КупиПродай, строка 9)",
            "• sold_база - сумма продаж в 'База товара' (КупиПродай, строка 10)",
            "• expected_diff - ожидаемая разница (sold_tovar - sold_база)",
            "• computed_diff - вычисленная разница = NOT_FOUND сумма - БАЗА_EXTRA сумма",
            "",
            "═══ КАТЕГОРИИ ПОЗИЦИЙ ═══",
            "",
            "• matched - позиции совпали полностью",
            "• tolerance - совпало с допуском (опечатки в году, прочерк/пусто, разница в цене)",
            "• wrong_date - совпало по A-H, но дата продажи отличается (в пределах 1 мес.)",
            "• date_anomaly - ⚠️ совпало, но даты отличаются на НЕСКОЛЬКО месяцев!",
            "• база_no_date - ★ совпало, но в Базе ПУСТАЯ дата продажи",
            "• potential - возможное совпадение (то же название и дата, но другие колонки)",
            "• not_found - позиция из tovar не найдена в Базе (добавляет к computed_diff)",
            "• база_extra - позиция в Базе не найдена в tovar (вычитает из computed_diff)",
            "",
        ],
        'All Issues': [
            "ВСЕ ПРОБЛЕМЫ",
